...
export default function ResultPage() {
  ...
  useEffect(() => {
    if (quiz && answers) {
      let correctCount = 0;
      quiz.questions.forEach((question) => {
        const userAnswer = answers[question.id]?.selectedAnswer; // Access selectedAnswer
        if (
          userAnswer &&
          userAnswer.toString().trim().toLowerCase() ===
            question.correctAnswer.toString().trim().toLowerCase()
  ...
  const isAnswerCorrect = (questionId) => {
    const userAnswer = answers[questionId]?.selectedAnswer; // Access selectedAnswer
    const correctAnswer = quiz.questions.find((q) => q.id === questionId)?.correctAnswer;
    if (!userAnswer || !correctAnswer) return false;
    return userAnswer.toString().trim().toLowerCase() === correctAnswer.toString().trim().toLowerCase();
  };
